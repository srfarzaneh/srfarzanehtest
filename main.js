...
function productCardHTML(product) {
  const modelsHTML = product.models
    .map((m) => `<span class="model-pill" dir="ltr">${m}</span>`)
    .join("");
  const modelsBlock = product.models.length
    ? `<div class="model-list" aria-label="مدل‌های موجود">${modelsHTML}</div>`
    : "";
  const nameEnHTML = product.nameEn
    ? `<span class="product-name-en ltr" dir="ltr">${product.nameEn}</span>`
    : "";
...
function initProductCount() {
  document.querySelectorAll("[data-product-count]").forEach((el) => {
    el.textContent = PRODUCTS.length.toLocaleString("fa-IR");
  });
}
